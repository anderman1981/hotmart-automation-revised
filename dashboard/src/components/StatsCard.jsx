...
import { TrendingUp, TrendingDown, DollarSign, Package, FileText, Users } from 'lucide-react';
    ...
    return (
        ...
        >
            <div className="absolute -top-6 -right-6 p-4 opacity-5 group-hover:opacity-10 transition-opacity transform group-hover:scale-110 duration-500 rounded-full bg-white/10">
                {Icon && <Icon className="w-30 h-30 text-white/20" />}
            </div>

            <div className="relative z-10">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-zinc-400 text-sm font-medium">{title}</h3>
                    {Icon && <div className="p-2 rounded-lg bg-white/5 text-zinc-400"><Icon className="w-5 h-5" /></div>}
                </div>
